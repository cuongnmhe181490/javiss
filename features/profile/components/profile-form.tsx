  ...
  return (
    <form
      ...
      onSubmit={async (event) => {
        ...
        if (!parsed.success) {
          setError(parsed.error.issues[0]?.message ?? "Vui long kiem tra lai cac truong trong ho so.");
          setIsPending(false);
          return;
        }
        ...
        try {
          ...
          setError(submissionError instanceof Error ? submissionError.message : "Khong the luu ho so.");
        } finally {
          setIsPending(false);
        }
      }}
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h2 className="mt-2 text-2xl font-semibold text-neutral-950">Tuy chon ca nhan hoa</h2>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Field label="Ten hien thi" name="displayName" defaultValue={initialValues.displayName} />
        <Field label="Tuoi" name="age" type="number" defaultValue={initialValues.age} />
        <SelectField label="Gioi tinh" name="sex" options={sexOptions} defaultValue={initialValues.sex ?? "prefer_not_to_say"} />
        <Field label="Chieu cao (cm)" name="heightCm" type="number" defaultValue={initialValues.heightCm ?? ""} />
        <Field label="Can nang (kg)" name="weightKg" type="number" defaultValue={initialValues.weightKg ?? ""} />
        <Field
        ...
        />
        <SelectField label="Muc tieu" name="goal" options={goalOptions} defaultValue={initialValues.goal} />
        <SelectField
        ...
        />
        <Field label="Dia diem" name="location" defaultValue={initialValues.location ?? ""} />
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <TextAreaField
          ...
          hint="Nhap cach nhau boi dau phay, vi du: high_protein, low_cost, quick."
        />
        <TextAreaField
          label="Di ung"
          name="allergiesCsv"
          defaultValue={joinCsvList(initialValues.allergies)}
          hint="Nhap danh sach can tranh, cach nhau boi dau phay."
        />
        <TextAreaField
      ...
      <ChoiceGroup label="Thiet bi tap luyen san co">
