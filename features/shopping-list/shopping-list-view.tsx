  ...
  return (
    <div className="space-y-6">
      <section className="rounded-[2rem] border border-black/5 bg-white p-6 shadow-[0_20px_80px_rgba(15,23,42,0.08)]">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div className="space-y-2">
            <p className="text-xs font-medium uppercase tracking-[0.32em] text-slate-500">Shopping list</p>
            <h2 className="text-3xl font-semibold tracking-tight text-slate-950">Aggregated from the full plan</h2>
            <p className="max-w-2xl text-sm leading-6 text-slate-600">
              Pantry-aware quantities are merged, normalized, and grouped into practical grocery sections.
            </p>
          </div>
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <Stat label="Total items" value={shoppingList.items.length.toString()} />
        <Stat label="Already covered" value={shoppingList.totals.pantryCoveredItems.toString()} />
        <Stat label="Need to buy" value={shoppingList.totals.buyItems.toString()} />
      </section>
        ...
        {shoppingList.groupedByCategory.map((group) => (
          <section key={group.category} className="rounded-[2rem] border border-black/5 bg-white p-5 shadow-[0_14px_50px_rgba(15,23,42,0.06)]">
            <div className="mb-4 flex items-center justify-between">
              ...
              <p className="text-sm text-slate-500">{Math.round(group.totalBuyQuantity)} to buy</p>
            </div>
              ...
              {group.items.map((item) => (
                <article key={item.normalizedName} className="flex flex-col gap-2 rounded-2xl border border-slate-200 bg-slate-50 p-4 md:flex-row md:items-center md:justify-between">
                  ...
                  <div className="text-sm text-slate-600">
                    <p>
                      Need {item.buyQuantity} {item.unit}
                    </p>
                    <p>
